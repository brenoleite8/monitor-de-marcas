function tpassword_start(a){$(`#${a} button`).click(function(){var b=$(this).find("i"),c=$(this).prev();b.toggleClass("fa-eye-slash");b.toggleClass("fa-eye");"text"==c.attr("type")?c.attr("type","password"):c.attr("type","text")})}
function tpassword_enable_strong_validation(a,b){let c=[],e="";"undefined"!==typeof b.minLength&&0<b.minLength.value&&(c.push({regex:new RegExp(`.{${b.minLength.value},}`)}),e+=`<li class='tpassword-list-item'><i class='fa fa-check'></i> <i class='fa fa-times'></i> ${b.minLength.message}</li>`);"undefined"!==typeof b.requireNumbers&&b.requireNumbers.value&&(c.push({regex:/[0-9]/}),e+=`<li class='tpassword-list-item'><i class='fa fa-check'></i> <i class='fa fa-times'></i> ${b.requireNumbers.message}</li>`);
"undefined"!==typeof b.requireLowercase&&b.requireLowercase.value&&(c.push({regex:/[a-z]/}),e+=`<li class='tpassword-list-item'><i class='fa fa-check'></i> <i class='fa fa-times'></i> ${b.requireLowercase.message}</li>`);"undefined"!==typeof b.requireUppercase&&b.requireUppercase.value&&(c.push({regex:/[A-Z]/}),e+=`<li class='tpassword-list-item'><i class='fa fa-check'></i> <i class='fa fa-times'></i> ${b.requireUppercase.message}</li>`);"undefined"!==typeof b.requireSpecialChar&&b.requireSpecialChar.value&&
(c.push({regex:/[^A-Za-z0-9]/}),e+=`<li class='tpassword-list-item'><i class='fa fa-check'></i> <i class='fa fa-times'></i> ${b.requireSpecialChar.message}</li>`);$(`#${a}`).attr("showpopover","true");var k=$(`#${a}`)[0];k.addEventListener("keyup",function(){"true"==$(`#${a}`).attr("showpopover")&&($(`#${a}`).attr("showpopover","false"),$(k).popover({html:!0,placement:"top",container:"body",content:`
...
$($(`#${a}-tpassword-checklist .tpassword-list-item`)[d]).find(".fa-times").show())})};function tdialog_start(a,b){$.fn.modal.Constructor.prototype._enforceFocus=function(){};$(a).modal({backdrop:!0,keyboard:!0});if("undefined"!=typeof b)$(a).on("hidden.bs.modal",function(){b();tdialog_close(a.substring(1))});else $(a).on("hidden.bs.modal",function(){tdialog_close(a.substring(1))})}function tdialog_close(a){$(".modal-backdrop").last().remove();$("#"+a).modal("hide");$("body").removeClass("modal-open");setTimeout(function(){$("#"+a).remove()},300)};function tjquerydialog_start(a,b,c,e,k,d,f,l,p,h,g,m,q){var v="window-wrapper"==$(a).parent().attr("role")?"#"+$(a).parent().attr("id"):"body",w="Close";"undefined"!==typeof Application.translation&&"undefined"!==typeof Application.translation[Adianti.language].close&&(w=Application.translation[Adianti.language].close);$(a).dialog({modal:b,stack:!1,zIndex:2E3,draggable:c,closeText:w,resizable:e,closeOnEscape:m,height:d,width:k,appendTo:v,dialogClass:q,beforeClose:function(){return"function"==typeof g?
(g(),!1):!0},close:function(u,z){$(this).remove();$(".tooltip.fade").remove();u=$(this).attr("name");$("[window_name="+u+"]").remove()},buttons:h});$(".ui-dialog").last().focus();$(a).closest(".ui-dialog").css({zIndex:p});$(".ui-widget-overlay").css({zIndex:100});0<f&&$(a).closest(".ui-dialog").css({top:f+"px"});0<l&&$(a).closest(".ui-dialog").css({left:l+"px"})}
function tjquerydialog_block_ui(){$(document).ready(function(){$(".ui-dialog").css("pointer-events","none");$(".ui-dialog-content").css("opacity","0.5")})}function tjquerydialog_unblock_ui(){$(".ui-dialog").css("pointer-events","all");$(".ui-dialog-content").css("opacity","")};function tform_send_data(a,b,c,e,k){if(0<parseInt(k))setTimeout(function(){tform_send_data(a,b,c,e,0)},k);else try{if("#"==b.substr(0,1))var d=$(b),f=[],l=[];else d=$("form[name="+a+"] [name='"+b+"']"),f=$("form[name="+a+"] [name='"+b+"[]']"),l=$("form[name="+a+"] [widget='tmultifile'][name='file_"+b+"[]']");if(d.length||f.length||l){if("undefined"==typeof Adianti.formEventsCounter||Number.isNaN(Adianti.formEventsCounter))Adianti.formEventsCounter=0;if(0==Adianti.formEventsCounter)if(l.length)try{l[0].tmultifile.send_data(JSON.parse(c))}catch(u){l[0].tmultifile.setValue(c)}else if(d.length){if("thtmleditor"==
d.attr("widget"))d.summernote("code",c);else if("bdaterange"==d.attr("widget"))bdaterange_set_value("#"+d.attr("id"),c);else if("tcolor"==d.attr("widget"))c?d.data("picker").setColor(c):d.data("picker").setColor(null);else if("tarrowstep"==d.attr("widget"))c?tarrowstep_set_current(d[0].name,c):tarrowstep_clear(d[0].name);else if("tdate"==d.attr("widget"))tdate_set_value("#"+d.attr("id"),c);else if("bsignaturedrawcapture"==d.attr("widget"))bsignaturedrawcapture_set_value(d.attr("id"),c);else if("timagecropper"==
d.attr("widget"))d[0].timagecropper.setValue(c);else if("tfile"==d.attr("widget"))d[0].tfile.setValue(c);else if("tcombosearch"==d.attr("role")){if(d.val()!==c){var p=function(u){return/<[a-z][\s\S]*>/i.test(u.text)?$("<span>"+u.text+"</span>"):u.text};d.val(c).trigger("change.select2");d.hasClass("tcombo_disabled")&&tmultisearch_disable_field(a,b)}}else if("tdbuniquesearch"==d.attr("widget")){var h=e;tdbuniquesearch_set_value(a,b,c,function(){h&&(d.attr("exitaction")&&tform_events_hang_exec(d.attr("exitaction")),
d.attr("changeaction")&&tform_events_hang_exec(d.attr("changeaction")))});e=!1}else if("multisearch"==d.attr("component"))c&&(p=function(u){return/<[a-z][\s\S]*>/i.test(u.text)?$("<span>"+u.text+"</span>"):u.text},d.select2({templateResult:p,templateSelection:p}).val(c).trigger("change.select2"));else if("radio"==d.attr("type")||"checkbox"==d.attr("type"))if(d.prop("checked",!1),c){var g=d.filter('[value="'+c+'"]').prop("checked",!0);"LABEL"==g.parent().prop("tagName")&&(g.parent().parent().find("label").removeClass("active"),
g.parent().toggleClass("active"))}else"LABEL"==d.parent().prop("tagName")&&d.parent().parent().find("label").removeClass("active");else d.val(c);e&&(d.attr("exitaction")&&tform_events_hang_exec(d.attr("exitaction")),d.attr("changeaction")&&tform_events_hang_exec(d.attr("changeaction")))}else{if(f.length){if("checkbox"==f.attr("type"))if(f.prop("checked",!1),c){f.parent().parent().find("label").removeClass("active");var m=JSON.parse(c);$.each(m,function(u,z){u=f.filter('[value="'+z+'"]').prop("checked",
!0);"LABEL"==u.parent().prop("tagName")&&u.parent().toggleClass("active")})}else f.parent().parent().find("label").removeClass("active");else if("multientry"==f.attr("component")){if(c){f.empty();var q=JSON.parse(c);$.each(q,function(u,z){f.append($("<option/>").val(z).text(z))});f.val(q).trigger("change.select2")}}else if("multisearch"==f.attr("component")&&"tuniquesearch"!==f.attr("widget")){if(c)if(q=JSON.parse(c),p=function(u){return/<[a-z][\s\S]*>/i.test(u.text)?$("<span>"+u.text+"</span>"):
u.text},"tmultisearch"==f.attr("widget"))f.select2({templateResult:p,templateSelection:p}).val(q).trigger("change.select2");else{if("tdbmultisearch"==f.attr("widget")){var v=e;tdbmultisearch_set_value(a,b,c,function(){v&&(f.attr("exitaction")&&tform_events_hang_exec(f.attr("exitaction")),f.attr("changeaction")&&tform_events_hang_exec(f.attr("changeaction")))})}else $.each(q,function(u,z){u=$($(f)[u]).attr("id");tform_send_data(a,"#"+u,z,e)});e=!1}}else if(f.length&&c)if(q=JSON.parse(c),$(f).find("option").prop("selected",
!1),"tselect"==f.attr("widget")&&1==f.length)$.each(q,function(u,z){$(f).find("option").filter('[value="'+z+'"]').prop("selected",!0)}),"bselectcheck"==$(f).attr("role")&&bselectcheck_update_summary(f,$(f).attr("placeholder"));else{var w=e;$.each(q,function(u,z){var A=$($(f)[u]).attr("id");setTimeout(function(){tform_send_data(a,"#"+A,z,w)},0,a,A,z)});e=!1}e&&(f.attr("exitaction")&&tform_events_hang_exec(f.attr("exitaction")),f.attr("changeaction")&&tform_events_hang_exec(f.attr("changeaction")))}}else tform_events_queue_push(function(){tform_send_data(a,
b,c,e)})}}catch(u){console.log(u)}}function tform_send_data_by_id(a,b,c,e,k){0<$("form[name="+a+"] [id="+b+"]").length&&tform_send_data(a,"#"+b,c,e,k)}function tform_events_hang_exec(a){if("undefined"==typeof Adianti.formEventsCounter||Number.isNaN(Adianti.formEventsCounter))Adianti.formEventsCounter=0;Adianti.formEventsCounter++;a=a.replace("'callback'","tform_decrease_events_counter");Function(a)()}function tform_events_stop(a){Adianti.formEventsStop=a}
...
function ttable_reset_counter(a){var b=0;$(a).find("tr").each(function(c,e){c=$(e).find("input,select");$(e).data("row",b);0<c.length&&($.each(c,function(k,d){$(d).data("row",b)}),b++)})}function ttable_remove_row(a){setTimeout(function(){var b=$(a).closest("table"),c=b.find("tbody").find("tr:visible").length;tr=$(a).closest("tr");1==c&&(c=tr.clone(),tfieldlist_reset_fields(c,!0),tr.after($(c)));tr.remove();ttable_reset_counter(b)},10)}
...
function ttable_add_row(a,b,c){$("#"+a+" tbody").append(base64_decode(c))}function ttable_replace_row_by_id(a,b,c){0<$("#"+a+" tbody").find("#"+b).length?$("#"+a+" tbody").find("#"+b).replaceWith(base64_decode(c)):$("#"+a+" tbody").append(base64_decode(c))};function tfieldlist_reset_fields(a,b){var c=parseInt(1E8*Math.random());$(a).attr("id",c);var e=$(a).find("input,select,textarea"),k=[];$.each(e,function(d,f){d=$(f).attr("id");var l=$(f).attr("widget"),p=$(f).attr("role");if("undefined"!==typeof d){var h=d.split("_");h.pop();var g=h.join("_"),m=g+"_"+c;h=$(f).parent();0<=k.indexOf(m)&&(m=g+parseInt(100*Math.random())+"_"+c);k.push(m);if("tdate"==l||"ttime"==l||"tdatetime"==l){$(f).attr("id",m);b&&$(f).val("");"undefined"!==typeof $(f).attr("exitaction")&&
$(f).attr("exitaction",$(f).attr("exitaction").replace(d,m));"undefined"!==typeof $(f).attr("onblur")&&$(f).attr("onblur",$(f).attr("onblur").replace(d,m));"undefined"!==typeof $(f).attr("changection")&&$(f).attr("changection",$(f).attr("changection").replace(d,m));"undefined"!==typeof $(f).attr("onchange")&&$(f).attr("onchange",$(f).attr("onchange").replace(d,m));grandparent=$(h).parent();f=$(f).detach();$(h).remove();grandparent.append(f);p=l;"ttime"==l&&(p="tdatetime");var q=new RegExp(d,"g");
tfieldlist_execute_scripts(grandparent,p,function(u){return u=u.replace(q,m)})}else if("tentry"==l)h=$(f).closest("td"),$(f).attr("id",m),b&&($(f).attr("name").match("/__row__id/")||$(f).val("")),"undefined"!==typeof $(f).attr("exitaction")&&$(f).attr("exitaction",$(f).attr("exitaction").replace(d,m)),"undefined"!==typeof $(f).attr("onblur")&&$(f).attr("onblur",$(f).attr("onblur").replace(d,m)),q=new RegExp(d,"g"),tfieldlist_execute_scripts(h,"tentry",function(u){return u=u.replace(q,m)});else if("tspinner"==
l)$(f).attr("id",m),b&&$(f).val(""),"undefined"!==typeof $(f).attr("exitaction")&&$(f).attr("exitaction",$(f).attr("exitaction").replace(d,m)),"undefined"!==typeof $(f).attr("onblur")&&$(f).attr("onblur",$(f).attr("onblur").replace(d,m)),grandparent=$(h).parent(),f=$(f).detach(),$(h).remove(),grandparent.append(f),q=new RegExp(d,"g"),tfieldlist_execute_scripts(grandparent,"tspinner",function(u){return u=u.replace(q,m)});else if("tcolor"==l)$(f).attr("id",m),b&&$(f).val(""),"undefined"!==typeof $(f).attr("exitaction")&&
$(f).attr("exitaction",$(f).attr("exitaction").replace(d,m)),"undefined"!==typeof $(f).attr("onblur")&&$(f).attr("onblur",$(f).attr("onblur").replace(d,m)),grandparent=$(h).parent(),grandparent.find(".pickr").replaceWith('<span class="input-group-addon tcolor"><i class="tcolor-icon"></i></span>'),q=new RegExp(d,"g"),tfieldlist_execute_scripts(grandparent,"tcolor",function(u){return u=u.replace(q,m)});else if("thidden"==l)$(f).attr("id",m),"true"==$(f).attr("uniqid")?$(f).val(parseInt(1E10*Math.random())):
b&&$(f).val(""),q=new RegExp(d,"g"),tfieldlist_execute_scripts(h,"thidden",function(u){return u=u.replace(q,m)});else if("ttext"==l)h=$(f).closest("td"),$(f).attr("id",m),b&&($(f).attr("name").match("/__row__id/")||$(f).val("")),"undefined"!==typeof $(f).attr("exitaction")&&$(f).attr("exitaction",$(f).attr("exitaction").replace(d,m)),"undefined"!==typeof $(f).attr("onblur")&&$(f).attr("onblur",$(f).attr("onblur").replace(d,m)),q=new RegExp(d,"g"),tfieldlist_execute_scripts(h,"ttext",function(u){return u=
u.replace(q,m)});else if("tdbmultisearch"==l||"tdbuniquesearch"==l)$(f).attr("id",m),$(f).removeData("select2-id").removeAttr("data-select2-id").find("option").removeAttr("data-select2-id"),$(a).removeData("select2-id").removeAttr("data-select2-id"),$(h).find(".select2-container").remove(),q=new RegExp(d,"g"),tfieldlist_execute_scripts(h,"tdbmultisearch",function(u){return u=u.replace(q,m)}),b&&setTimeout(function(){$(f).val("").trigger("change.select2")},10);else if("tmultisearch"==l||"tuniquesearch"==
l)$(f).attr("id",m),$(f).removeData("select2-id").removeAttr("data-select2-id").find("option").removeAttr("data-select2-id"),$(a).removeData("select2-id").removeAttr("data-select2-id"),$(h).find(".select2-container").remove(),q=new RegExp(d,"g"),tfieldlist_execute_scripts(h,"tmultisearch",function(u){return u=u.replace(q,m)}),b&&setTimeout(function(){$(f).val("").trigger("change.select2")},10);else if("tcombo"==l)$(f).attr("id",m),b&&$(f).val(""),"tcombosearch"==p&&($(f).removeData("select2-id").removeAttr("data-select2-id").find("optgroup").removeAttr("data-select2-id"),
$(f).removeData("select2-id").removeAttr("data-select2-id").find("option").removeAttr("data-select2-id"),$(f).removeData("select2-id").removeAttr("data-select2-id").find("optgroup").find("option").removeAttr("data-select2-id"),$(a).removeData("select2-id").removeAttr("data-select2-id"),$(h).find(".select2-container").remove()),"undefined"!==typeof $(f).attr("changeaction")&&$(f).attr("changeaction",$(f).attr("changeaction").replace(d,m)),"undefined"!==typeof $(f).attr("onchange")&&$(f).attr("onchange",
$(f).attr("onchange").replace(d,m)),q=new RegExp(d,"g"),tfieldlist_execute_scripts(h,"tcombo",function(u){return u=u.replace(q,m)});else if("tfile"==l){$(f).attr("id",m);b&&$(f).val("");h=$(f).closest("td");p=h.find(".div_file").attr("id");var v="div_file_"+m;h.find(".tfile_row_wrapper").remove();h.find(".file-response-icon").remove();h.find(".div_file").attr("id",v);$(f).css("padding-left","5px");"undefined"!==typeof $(f).attr("changeaction")&&$(f).attr("changeaction",$(f).attr("changeaction").replace(d,
m));"undefined"!==typeof $(f).attr("onchange")&&$(f).attr("onchange",$(f).attr("onchange").replace(d,m));q=new RegExp(d,"g");var w=new RegExp(p,"g");tfieldlist_execute_scripts(h,"tfile",function(u){u=u.replace(q,m);return u=u.replace(w,v)})}}"tradiobutton"==l?$(f).parent().removeClass("active"):"tcheckbutton"==l&&$(f).parent().removeClass("active")});$(a).find("[name$='__row__id[]']").val(c)}
function tfieldlist_execute_scripts(a,b,c){a=$(a).find("script");$.each(a,function(e,k){e=$(k).text();e=c(e);e.trim().split("_")[0]==b&&$(k).replaceWith("<script>"+e+"\x3c/script>")})}
...
function tfieldlist_clear_rows(a,b,c){var e=$("[name="+a+"] tbody").find("tr").length;c=(0==c?e:c)+b;c>=e&&(ttable_clone_previous_row($("[name="+a+"] tbody").find("tr:last")),c=e);$("[name="+a+"] tbody").find("tr").each(function(k,d){k>=b&&k<c&&$(d).remove()})}
...
function tfieldlist_update_sum(a,b,c){setTimeout(function(){var e=tfieldlist_column_sum(b),k=$("[name^="+b+"]:first").data("nmask"),d=[],f=$("table[name="+a+"]").closest("form");k&&(d=k.split(""));k=$("[name=grandtotal_"+b+"]");e=number_format(e,d[0]??2,d[1]??",",d[2]??".");k.val(e);k.data("total-form-field")&&(d=k.data("total-form-field"),f.find('[name="'+d+'"]').val(e));c&&"function"===typeof c&&c();f=$("table[name="+a+"]").attr("total-update-action");if("undefined"!==typeof f&&0<f.length){if(d=
$("table[name="+a+"]").closest("form")){var l={};e={};d=$(d).serializeArray();$(d).each(function(p,h){p=h.name;p=p.replace("[]","");void 0===l[p]&&(l[p]=[]);l[p].push(h.value)});for(prop in l)if(1<=l[prop].length)for(d=0;d<l[prop].length;d++)"undefined"==typeof e[d]&&(e[d]={}),e[d][prop]=l[prop][d];l.list_data=e}__adianti_post_exec(f,l,null,"1",!0)}},50)}
function tfieldlist_update_count(a,b,c){setTimeout(function(){var e=tfieldlist_column_count(b),k=$("[name^="+b+"]:first").data("nmask"),d=[];k&&(d=k.split(""));$("[name=grandtotal_"+b+"]").val(number_format(e,d[0]??2,d[1]??",",d[2]??"."));c&&"function"===typeof c&&c();e=$("table[name="+a+"]").attr("total-update-action");if("undefined"!==typeof e&&0<e.length){if(d=$("table[name="+a+"]").closest("form")){var f={};k={};d=$(d).serializeArray();$(d).each(function(l,p){l=p.name;l=l.replace("[]","");void 0===
f[l]&&(f[l]=[]);f[l].push(p.value)});for(prop in f)if(1<=f[prop].length)for(d=0;d<f[prop].length;d++)"undefined"==typeof k[d]&&(k[d]={}),k[d][prop]=f[prop][d];f.list_data=k}__adianti_post_exec(e,f,null,"1",!0)}},50)}function tfieldlist_enable_field(a){try{0<$('table.tfieldlist[name="'+a+'"]').length&&$('table.tfieldlist[name="'+a+'"]').parent().find(".tfieldlist-disable").remove()}catch(b){console.log(b)}}
...
function tdatagrid_update_total(a){var b=a.substring(1),c=document.querySelector(a),e=$(a).closest("form");(new MutationObserver(function(k){k.forEach(function(d){if("TBODY"==d.target.tagName){d=$(a).find("[data-total-function=sum]");var f=$(a).find("[data-total-function=count]");d.each(function(l,p){var h=0,g=$(p).data("column-name"),m=$(p).data("total-mask"),q=m.split(":");0<q.length?(l=q[0],m=q[1]):(l="",m=m.substring(1));$('[name="'+b+"_"+g+'[]"]').each(function(v,w){h+=parseFloat($(w).val())});
g=number_format(h,m.substring(0,1),m.substring(1,2),m.substring(2,3));h&&($(p).html(l+" "+g),$(p).data("value",h),$(p).attr("data-value",h));$(p).data("total-form-field")&&(p=$(p).data("total-form-field"),e.find('[name="'+p+'"]').val(g))});f.each(function(l,p){var h=$(p).data("column-name");l=$('[name="'+b+"_"+h+'[]"]').length;var g=$(p).data("total-mask"),m=g.split(":");if(0<m.length){var q=m[0];g=m[1]}else q="",g=g.substring(1);$('[name="'+b+"_"+h+'[]"]').each(function(v,w){total+=parseFloat($(w).val())});
h=number_format(total,g.substring(0,1),g.substring(1,2),g.substring(2,3));total&&($(p).html(q+" "+h),$(p).data("value",total),$(p).attr("data-value",total));l&&($(p).html(l),$(p).data("value",l),$(p).attr("data-value",l));$(p).data("total-form-field")&&(p=$(p).data("total-form-field"),$('[name="'+p+'"]').val(l))})}})})).observe(c,{attributes:!0,childList:!0,characterData:!0,subtree:!0,attributeOldValue:!0,characterDataOldValue:!0})}
function tdatagrid_mutation_action(a,b){var c=a.substring(1),e=document.querySelector(a);(new MutationObserver(function(k){k.forEach(function(d){var f={},l={};if("TBODY"==d.target.tagName){$(a).find("[data-hidden-field=true]").each(function(h,g){h=$(g).attr("name").replace(c+"_","");h=h.replace("[]","");"="!==h.substring(0,1)&&("undefined"==typeof f[h]&&(f[h]=[]),f[h].push($(g).val()))});for(prop in f)for(d=0;d<f[prop].length;d++)"undefined"==typeof l[d]&&(l[d]={}),l[d][prop]=f[prop][d];var p={};
...
function tdatagrid_show_properties(a,b){const c=$(a),e=c.closest("table"),k=e.attr("id");console.log(e);if(c.data("bs.popover"))c.popover("toggle");else{var d=[];e.find("th.tdatagrid_col").each(function(){const f=$(this),l=f.text().replace(/[\n\r]+|[\s]{2,}/g," ").trim(),p=f.index(),h=!f.is(":hidden");l&&d.push({value:f.attr("data-column-id"),name:l,index:p,visible:h})});a=`
...
$(this);const g=h.data("column");h=h.prop("checked");tdatagrid_toggle_column_visibility(e,g,h)});const l=f.find(".page-size-select").val(),p=f.find(".group-column-select").val();tdatagrid_save_preferences(k,{page_limit:l,groups:p,columns:getColumnsVisibility(f)});c.popover("hide");__adianti_post_exec(b,{page_limit:l,groups:p,columns:getColumnsVisibility(f)},null,!0,!0)})});c.on("hidden.bs.popover",function(){c.popover("dispose")});setTimeout(()=>c.popover("show"),10)}}
function getColumnsVisibility(a){const b=[];a.find(".column-visibility").each(function(){const c=$(this);b.push({columnId:c.prop("value"),visible:c.prop("checked")})});return b}function tdatagrid_toggle_column_visibility(a,b,c){a=a.find(`tr > *:nth-child(${b+1})`);c?a.show():a.hide()}function tdatagrid_start_hide_columns(a){$(`${a} tr th[data-start-hide='true']`).each(function(b){b=$(this).index();$(this).hide();$(`${a} tr td:nth-child(${b+1})`).hide()})}
function tdatagrid_save_preferences(a,b){window.localStorage&&localStorage.setItem(`datagrid_preferences_${a}`,JSON.stringify(b))}
function tdatagrid_init_header_popover_filters(a){setTimeout(()=>{const b=$(`#${a}`);b.find("#datagrid-header-filter-row").hide();b.find("#datagrid-header-filter-row td").each(function(){const c=$(this),e=c.find("input, select");if(0!==e.length){var k=b.find("thead tr th").eq(c.index()),d=$('<i class="fas fa-filter filter-icon" style="margin-right: 5px; cursor: pointer;"></i>');k.prepend(d);k=e.first().attr("widget");var f=e.first().attr("changeaction"),l=e.first().attr("exitaction");if("bdaterange"===
k){const h=e.first().closest(".bdaterange-container");f=h.data("picker-options")?.changeaction;e.off("change");h.data("picker")?.off("select click");h.data("event-enabled",!1)}e.data("changeaction",f);e.data("exitaction",l);"tcombo"!==k||"tcombosearch"!==e.attr("role")&&"bselectcheck"!==e.attr("role")?e.removeAttr("onchange onblur changeaction exitaction").off("change blur"):e.off("change.changeaction");var p=e.first().attr("id");d.data("filter_id",p);d.popover({html:!0,content:function(){if("bselectcheck"===
e.attr("role")){var h=$('<div style="display: flex; align-items: center; min-width: 400px;"></div>'),g=$(`<div style="width: 100%;" id="filter-container-${p}"></div>`);c.contents().appendTo(g);h.append(g);g=$('<button class="btn btn-sm btn-primary apply-filter" style="margin-left: 8px;"><i class="fas fa-check"></i> Aplicar</button><button class="btn btn-sm btn-default clear-filter" style="margin-left: 4px;"><i class="fas fa-eraser"></i></button>');h.append(g);h.find(".apply-filter").on("click",function(){d.popover("hide");
f&&eval(f);l&&eval(l)});h.find(".clear-filter").on("click",function(){e.val("");d.popover("hide");f&&eval(f);l&&eval(l)});return h}h=$('<div class="filter-container" style="min-width: 250px; display: flex; flex-direction: column; align-items: center;"></div>');g=$(`<div style="min-width: 250px; display: flex; flex-direction: column; align-items: center;" id="filter-container-${p}"></div>`);c.contents().appendTo(g);h.append(g);g=$('<div style="margin-top: 10px; text-align: center; width: 100%; display: flex; justify-content: center; gap: 8px;"><button class="btn btn-sm btn-primary apply-filter"><i class="fas fa-check"></i> Aplicar</button><button class="btn btn-sm btn-default clear-filter"><i class="fas fa-times"></i> Limpar</button></div>');
g.find(".apply-filter").on("click",function(){d.popover("hide");f&&eval(f);l&&eval(l)});g.find(".clear-filter").on("click",function(){"bdaterange"===e.first().attr("widget")?e.first().closest(".bdaterange-container").data("picker").clear():e.val("");d.popover("hide");f&&eval(f);l&&eval(l)});h.append(g);return h},placement:"auto",trigger:"manual",container:d.closest("form")}).on("hide.bs.popover",function(){$(`#filter-container-${p}`).contents().appendTo(c)}).on("shown.bs.popover",function(){$(".popover").css("max-width",
"500px")});d.on("click",function(h){h.stopPropagation();h.preventDefault();h.stopImmediatePropagation();d.popover("toggle")})}});tdatagrid_update_filter_icons(a);tdatagrid_update_filter_badges(a)},1)}
...
$(this).find("input, select");if(0!==d.length){k=b.find("thead tr th").eq(k).text().trim();var f=d.val();if(Array.isArray(f)&&0<f.length||!Array.isArray(f)&&null!=f&&""!==f){e=!0;let l=f;"bdaterange"===d.first().attr("widget")?l=`${d.first().val()} - ${d.eq(1).val()}`:Array.isArray(f)?l=d.find("option:selected").map(function(){return $(this).text()}).get().join(" - "):d.is("select")&&(l=d.find("option:selected").text());k=$('<div style="border: 1px solid var(--button-border); border-radius: 4px; padding: 4px 8px; display: flex; align-items: center; gap: 4px; font-size: 12px;">'+
`<span>${k}: <b>${l}</b></span>`+'<span title="Remover filtro" class="btn btn-sm btn-link remove-filter" style="padding: 0 2px; font-size: 12px; line-height: 1; background: none; border: none; box-shadow: none; min-height: 15px; height: 15px; color: var(--button-text);">\u00d7</span></div>');k.find(".remove-filter").on("click",function(){"bdaterange"===d.first().attr("widget")?d.first().closest(".bdaterange-container").data("picker").clear():d.val("");const p=d.data("changeaction"),h=d.data("exitaction");
p?eval(p):h&&eval(h)});c.append(k)}}});b.before(a);a.toggle(e)}function tdatagrid_update_filter_icons(a){const b=$(`#${a}`);b.find("#datagrid-header-filter-row td").each(function(c){var e=$(this).find("input, select");0!==e.length&&(c=b.find("thead tr th").eq(c).find(".filter-icon"),e=e.val(),c.css("color",Array.isArray(e)&&0<e.length||!Array.isArray(e)&&null!=e&&""!==e?"var(--main-color)":""))})};function tbutton_enable_field(a,b){setTimeout(function(){$("button[name="+b+"]").removeAttr("disabled");$("button[name=btn_"+b+"]").removeAttr("disabled");$("#tbutton_"+b).removeAttr("disabled")},1)}function tbutton_disable_field(a,b){setTimeout(function(){$("button[name="+b+"]").attr("disabled",!0);$("button[name=btn_"+b+"]").attr("disabled",!0);$("#tbutton_"+b).attr("disabled",!0)},1)};function tcheckgroup_enable_field(a,b){setTimeout(function(){$("form[name="+a+"] [checkgroup="+b+"]").attr("onclick",null);$("form[name="+a+"] [checkgroup="+b+"]").css("pointer-events","auto");$("form[name="+a+"] [checkgroup="+b+"]").closest(".toggle-wrapper").removeClass("tfield_block_events")},1)}
...
l.on("save",function(p,h){const g=f.val();p=p?.toHEXA().toString();f.val(p);h.hide();g!=p&&"undefined"!=typeof e&&e(p)});l.on("swatchselect",function(p,h){h.applyColor()});f.on("change",function(){var p=-1==this.value.indexOf("#")?"#":"";this.value?l.setColor(p+this.value,!1):l.setColor(null)})};function ticon_enable_field(a,b){try{setTimeout(function(){$("form[name="+a+'] div[name="block_'+b+'"]').remove()},1)}catch(c){console.log(c)}}
...
function ticon_start(a,b){if(b)$("#"+a).iconpicker().on("iconpickerUpdated",function(c){b($(this).val())});else $("#"+a).iconpicker()};function tcombo_enable_field(a,b){form_name_sel="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var c='[name="'+b+'"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(c="#"+b);try{"tcombosearch"==$(form_name_sel+c).attr("role")?tmultisearch_enable_field(a,b):($(form_name_sel+c).attr("onclick",null),$(form_name_sel+c).css("pointer-events","auto"),$(form_name_sel+c).removeClass("tcombo_disabled"))}catch(e){console.log(e)}}
...
function tcombo_add_option(a,b,c,e){c=c.replace(/"/g,"");a="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var k='select[name="'+b+'"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(k="#"+b);b=$(a+k).find("optgroup");0<b.length?$('<option value="'+c+'">'+e+"</option>").appendTo(b.last()):$('<option value="'+c+'">'+e+"</option>").appendTo(a+k)}
function tcombo_create_opt_group(a,b,c){a="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var e='[name="'+b+'"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(e="#"+b);$('<optgroup label="'+c+'"></optgroup>').appendTo(a+e)}
function tcombo_clear(a,b,c){"undefined"==typeof c&&(c=!0);a="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var e='[name="'+b+'"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(e="#"+b);b=$(a+e);"tcombosearch"==b.attr("role")?0<b.find("option:not(:disabled)").length&&b.val("").empty().trigger("change.select2"):(b.val(!1),b.html(""));c&&b.attr("changeaction")&&tform_events_hang_exec(b.attr("changeaction"))}
...
function tcombo_set_quick_register_value(a,b){a=$(a).val();$("#"+b+"_quickregister").val(a);$("#"+b+"_btn").attr("data-quick_register_value",a)}function select2_prepare_no_results(a,b){if("undefined"!=typeof $(b).attr("noresultsbtnprops")){const c=JSON.parse(base64_decode($(b).attr("noresultsbtnprops")));a.language={noResults:function(){return`
                    <span>${c.noResultsMessage||$.fn.select2.defaults.defaults.language.noResults()}</span>
                    <div class="no-results-wrapper">
                        ${c.btn}
                    </div>
                `}}}if("undefined"!=typeof $(b).attr("noresultsquickregisterprops")){const c=JSON.parse(base64_decode($(b).attr("noresultsquickregisterprops")));a.language={noResults:function(){const e=c.noResultsMessage||$.fn.select2.defaults.defaults.language.noResults(),k=$(b).data("select2").$dropdown.find(".select2-search__field").val();let d=c.input,f=c.btn;k&&(d=d.replace('value=""','value="'+k+'"'),f=f.replace('data-quick_register_value=""','data-quick_register_value="'+k+'"'));return`
                    <span>${e}</span>
                    <div class="no-results-wrapper">
                        ${d}
                        ${f}
                    </div>
                `}}}if("undefined"!=typeof $(b).attr("noresultsquickregisterprops"))$(b).on("select2:open",function(){$(this).data("select2").$dropdown.find(".select2-search__field").off("input").on("input",function(c){c=$(this).val();$(b+"_quickregister").val(c);$(b+"_btn").attr("data-quick_register_value",c)})})};function tdate_enable_field(a,b){try{$('form[name="'+a+'"] [name="'+b+'"]').removeClass("tfield_disabled"),$('form[name="'+a+'"] [name="'+b+'"]').css("pointer-events","auto"),$('form[name="'+a+'"] [name="'+b+'"]').removeAttr("tabindex"),$('form[name="'+a+'"] [name="'+b+'"]').attr("readonly",!1)}catch(c){console.log(c)}setTimeout(function(){$('form[name="'+a+'"] [name="'+b+'"]').next().show()},1)}
...
function tentry_mask(a,b,c){var e=a.value;if("undefined"==typeof e)return!0;"1"==$(a).attr("forceupper")&&(e=e.toUpperCase());"1"==$(a).attr("forcelower")&&(e=e.toLowerCase());keyCode=document.all?b.keyCode:b.which;if(8==keyCode||9==b.keyCode||13==b.keyCode)return!0;var k="";var d=0;var f="!"==c.charAt(1)?a.value.length+1:c.length;for(b=0;b<f-1;b++)maskChar=c.charAt(b),valueChar=e.charAt(d),b<=e.length&&(("-"==maskChar||"_"==maskChar||"."==maskChar||"/"==maskChar||"\\"==maskChar||":"==maskChar||"|"==
maskChar||"("==maskChar||")"==maskChar||"["==maskChar||"]"==maskChar||"{"==maskChar||"}"==maskChar)&maskChar!==valueChar?k+=maskChar:(k+=valueChar,d++));a.value=k;a=a.value.length;maskChar="!"==c.charAt(1)?c.charAt(0):c.charAt(a);switch(maskChar){case "A":case "a":return 47<keyCode&&58>keyCode||64<keyCode&&91>keyCode||96<keyCode&&123>keyCode;case "S":case "s":return 64<keyCode&&91>keyCode||96<keyCode&&123>keyCode;case "9":return 47<keyCode&&58>keyCode}return!0}
...
function tentry_autocomplete(a,b,c){var e='input[name="'+a+'"]';0<$("#"+a).length&&(e="#"+a);c=Object.assign({lookup:b,onSelect:function(){var k=$(this).data("val"),d=$(this).val();$(this).data("val",d);$(e).removeAttr("onblur");k!==d&&$(e).attr("exitaction")&&(string_callback=$(e).attr("exitaction"),Function(string_callback)())}},JSON.parse(c));$(e).autocomplete(c)}function tentry_autocomplete_by_name(a,b,c){objectId=$("[name="+a+"]").attr("id");tentry_autocomplete(objectId,b,c)}
function tentry_numeric_mask(a,b,c,e,k,d){var f='input[name="'+a+'"]';0<$("#"+a).length&&(f="#"+a);$(f).maskMoney({prefix:"",suffix:"",affixesStay:!0,thousands:e,decimal:c,precision:b,allowZero:!0,allowNegative:d,formatOnBlur:k,reverse:k,bringCaretAtEndOnFocus:!k,selectAllOnFocus:!1,allowEmpty:!0})}
...
function tentry_change_mask(a,b,c){form_name_sel="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var e='[name="'+b+'"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(e="#"+b);$(document).ready(function(){c=c.replace(/9/g,"0");var k=0<$(e).attr("maxlength")?$(e).attr("maxlength"):250;"A!"==c?c="A".repeat(k):"S!"==c?c="S".repeat(k):"0!"==c&&(c="0".repeat(k));$(e).mask(c)})}
...
A.delFile==A.newFile&&(A="");h.setData(A)});return q};this.clear=function(){h.file_row_wrapper&&h.file_row_wrapper.hide();h.parent_container.children("i").attr({"class":"",title:""});h.parent_container.find("[widget=tfile]").css("padding-left","5px");$(h.file_input_hidden).val("");$("#"+this.input_id).val("")};this.showFile=function(g){"undefined"==typeof g&&(g=this.getData());if(g=g.fileName){var m=h.file_row_wrapper,q=m.find(".tfile_link_wrapper");m.find(".progress-bar").css("width","100%");if(-1<
$.inArray(g.split(".").pop().toLowerCase(),"png jpg jpeg gif svg webp ico fav".split(" "))){m='<img style="max-width:460px" src="download.php?file={file_name}">';h.popover.content&&(m=atob(h.popover.content));m=m.replace("{file_name}",g);if("1"==h.image_gallery.enabled){var v=$("<a />",{href:"download.php?file="+g,target:"_blank"}),w=$("<img />",{src:"download.php?file="+g}).height(h.image_gallery.height).width(h.image_gallery.width);v.append(w)}else v=$("<a />",{href:"download.php?file="+g,target:"_blank"});
"1"==h.popover.enabled&&v.popover({trigger:"hover",content:m,html:!0,container:"body",template:'<div class="popover" role="tooltip" style="z-index:100000;max-width:800px"><div class="arrow"></div><h3 class="popover-title popover-header"></h3><div class="popover-content popover-body"><div class="data-content"></div></div></div>',title:h.popover.title||""})}else v=$("<a />",{href:"download.php?file="+g,target:"_blank"});h.file_row_wrapper.show();"1"!=h.image_gallery.enabled&&v.append(g);q.html(v)}};
this.getData=function(){var g=decodeURIComponent(h.file_input_hidden.val()),m="";try{m=g?JSON.parse(g):{}}catch(q){m={}}return m};this.setData=function(g){h.clear();g&&$(h.file_input_hidden).val(encodeURIComponent(JSON.stringify(g)))};this.supportAjaxUploadWithProgress=function(){return this.supportFileAPI()&&this.supportAjaxUploadProgressEvents()&&this.supportFormData()};this.supportFileAPI=function(){var g=document.createElement("INPUT");g.type="file";return"files"in g};this.supportAjaxUploadProgressEvents=
...
this.onLoadStart,!1);q.upload.addEventListener("progress",this.onProgress,!1);q.upload.addEventListener("load",this.onLoad,!1);q.addEventListener("readystatechange",this.onReadyStateChange,!1);q.open("POST",m,!0);q.send(g)};this.onLoadStart=function(g){0==h.parent_container.children("i").length&&h.parent_container.prepend($("<i>"));h.file_handling&&h.file_row_wrapper.show();h.parent_container.find("[widget=tfile]").css("padding-left","20px");h.parent_container.children("i").attr({"class":"fa fa-spinner fa-spin fa-fw blue file-response-icon",
title:"..."})};this.onLoad=function(g){};this.onProgress=function(g){g.lengthComputable&&h.file_handling&&(g=Math.round(100*g.loaded/g.total),h.file_progress_bar.css("width",g+"%").css("height",2),h.file_link_wrapper.html(g+"%"))};this.onReadyStateChange=function(g){var m=null;try{m=g.target.status}catch(u){return}if("200"==m&&"4"==g.target.readyState&&g.target.responseText)try{var q=JSON.parse(g.target.responseText);if("success"==q.type){if(h.file_handling){var v=h.getData();v.fileName&&(v.delFile=
v.fileName);v.newFile="tmp/"+q.fileName;v.fileName="tmp/"+q.fileName;h.setData(v);h.showFile(v)}else{var w=$("#"+h.input_id).parent().children("input[type=hidden]");$(w).val(q.fileName)}h.showMessage("success",_t("success"));4==this.readyState&&"function"==typeof h.complete_action&&h.complete_action()}else h.file_handling&&h.file_row_wrapper.remove(),"function"==typeof h.error_action&&h.error_action(),h.showMessage("error",q.msg)}catch(u){"function"==typeof h.error_action&&h.error_action(),h.file_handling&&
h.file_row_wrapper.remove(),"undefined"==typeof q?h.showMessage("error",g.target.responseText):h.showMessage("error",u)}};this.showMessage=function(g,m){h.parent_container.children("i").removeAttr("data-original-title");this.drop_zone?"success"==g?(g=_t("file_uploaded_successfully"),$(`#tfile_dropzone_info_${this.input_id}`).html(`
                    <div class="alert alert-success" role="alert">${g}</div>    
                `).delay(3E3).fadeOut("slow",function(){$(this).empty().show()})):"error"==g&&$(`#tfile_dropzone_info_${this.input_id}`).html(`
                    <div class="alert alert-danger" role="alert">${m}</div>    
                `):"success"==g?(h.parent_container.find("[widget=tfile]").css("padding-left","20px"),h.parent_container.children("i").attr({"class":"far fa-check-circle green file-response-icon","data-original-title":m})):"error"==g&&(h.parent_container.find("[widget=tfile]").css("padding-left","20px"),h.parent_container.children("i").attr({"class":"fa fa-exclamation-circle red file-response-icon","data-original-title":m}))};this.setValue=function(g){this.clear();if(g){var m={fileName:g};0<=g.indexOf("%7B%")&&
...
if(0<q.length){q=q[0];const v=new DataTransfer;v.items.add(q);m[0].files=v.files;m.trigger("change")}});g.on("click",function(){m.click()});m.on("change",function(){});$(document).on("click",".tfile_del_icon",function(){})}}
function tfile_start(a,b,c,e,k,d,f,l,p,h){$(function(){var g=new FileUploader(a,b,c,e,k,d,f,l,p,h);$("#"+a).change(function(){try{g.initFileUpload()}catch(q){g.clear(),g.showMessage("error",q.message),"function"==typeof this.error_action&&g.error_action()}});if(d){var m=$("#"+a).parent().children("input[type=hidden]");g.createStatusRow();$(m).val()&&g.setData(JSON.parse(decodeURIComponent($(m).val())));g.showFile();$(m).remove()}h&&g.enableDropZone();$('[name="'+g.field_name+'"]')[0].tfile=g})}
...
{"class":"tfile_row_1"});var v=$("<div />",{"class":"tfile_row_2"});m=$("<div />",{"class":"tfile_link_wrapper",style:"width: calc("+m+" - 25px)"});var w=$("<span />",{"class":"tfile_del_icon fa fa-minus-circle gray",title:"Remover"}),u=$("<div />",{"class":"progress-bar progress-bar-success",style:"width: 0%; height: 2px;"}),z=$("<input />",{type:"hidden",name:h.field_name+"[]"});"1"==h.image_gallery.enabled&&($(m).css("width","unset"),$(q).css("border","1px solid gray"),$(q).css("border-radius",
"3px"),$(q).css("margin","2px"),$(q).css("display","inline-block"));h.file_row_wrapper=q;h.file_input_hidden=z;h.file_progress_bar=u;h.file_link_wrapper=m;h.parent_container.append(q);q.append(g);q.append(v);g.append(z,m,w);v.append(u);$(w).click(function(){var A=h.getData();q.hide();h.parent_container.children("i").attr({"class":"",title:""});h.parent_container.find("[widget=tmultifile]").css("padding-left","5px");A.delFile=A.fileName;A.delFile==A.newFile&&(A="",h.setData(A),q.remove());h.setData(A)});
return q};this.createInputHidden=function(g){g=$("<input />",{type:"hidden",name:h.field_name+"[]",widget:"thidden",value:g});h.parent_container.append(g);return g};this.showFile=function(g){"undefined"==typeof g&&(g=this.getData());if(g=g.fileName){var m=h.file_row_wrapper,q=m.find(".tfile_link_wrapper");m.find(".progress-bar").css("width","100%");if(-1<$.inArray(g.split(".").pop().toLowerCase(),"png jpg jpeg gif svg webp ico fav".split(" ")))if(m='<img style="max-width:460px" src="download.php?file={file_name}">',
h.popover.content&&(m=atob(h.popover.content)),m=m.replace("{file_name}",g),"1"==h.image_gallery.enabled){var v=$("<a />",{href:"download.php?file="+g,target:"_blank"}),w=$("<img />",{src:"download.php?file="+g}).height(h.image_gallery.height).width(h.image_gallery.width);v.append(w);"1"==h.popover.enabled&&q.popover({trigger:"hover",content:m,html:!0,container:"body",template:'<div class="popover" role="tooltip" style="z-index:100000;max-width:800px"><div class="arrow"></div><h3 class="popover-title popover-header"></h3><div class="popover-content popover-body"><div class="data-content"></div></div></div>',
title:h.popover.title||""})}else v=$("<a />",{href:"download.php?file="+g,target:"_blank"}),"1"==h.popover.enabled&&v.popover({trigger:"hover",content:m,html:!0,container:"body",template:'<div class="popover" role="tooltip" style="z-index:100000;max-width:800px"><div class="arrow"></div><h3 class="popover-title popover-header"></h3><div class="popover-content popover-body"><div class="data-content"></div></div></div>',title:h.popover.title||""});else v=$("<a />",{href:"download.php?file="+g,target:"_blank"});
"1"!=h.image_gallery.enabled&&v.append(g);q.html(v)}};this.getData=function(){var g=decodeURIComponent(h.file_input_hidden.val()),m="";try{m=g?JSON.parse(g):{}}catch(q){m={}}return m};this.setData=function(g){g?$(h.file_input_hidden).val(encodeURIComponent(JSON.stringify(g))):$(h.file_input_hidden).val("")};this.supportAjaxUploadWithProgress=function(){return this.supportFileAPI()&&this.supportAjaxUploadProgressEvents()&&this.supportFormData()};this.supportFileAPI=function(){var g=document.createElement("INPUT");
g.type="file";return"files"in g};this.supportAjaxUploadProgressEvents=function(){var g=new XMLHttpRequest;return!!(g&&"upload"in g&&"onprogress"in g.upload)};this.supportFormData=function(){return!!window.FormData};this.initFileUpload=function(){var g=new FormData,m=h.obj_file;g.append("fileName",m);if(h.limitSize&&(this.onLoadStart(),m.size>h.limitSize))throw Error(_t("large_file")+h.limitSize/1024/1024+"MBs");this.createStatusRow();this.supportAjaxUploadWithProgress()&&this.sendXHRequest(g,h.service_action)};
this.sendXHRequest=function(g,m){var q=new XMLHttpRequest;q.upload.addEventListener("loadstart",this.onLoadStart,!1);q.upload.addEventListener("progress",this.onProgress,!1);q.upload.addEventListener("load",this.onLoad,!1);q.addEventListener("readystatechange",this.onReadyStateChange,!1);q.open("POST",m,!0);q.send(g)};this.onLoadStart=function(g){0==h.parent_container.children("i").length&&h.parent_container.prepend($("<i>"));h.parent_container.find("[widget=tmultifile]").css("padding-left","20px");
h.parent_container.children("i").attr({"class":"fa fa-spinner fa-spin fa-fw blue file-response-icon","data-original-title":"...",title:"..."})};this.onLoad=function(g){};this.onProgress=function(g){g.lengthComputable&&(g=Math.round(100*g.loaded/g.total),h.file_progress_bar.css("width",g+"%").css("height",2),h.file_link_wrapper.html(g+"%"))};this.onReadyStateChange=function(g){var m=null;try{m=g.target.status}catch(w){return}if("200"==m&&"4"==g.target.readyState&&g.target.responseText)try{var q=JSON.parse(g.target.responseText);
if("success"==q.type){if(h.file_handling){var v=h.getData();v.newFile="tmp/"+q.fileName;v.fileName="tmp/"+q.fileName;h.setData(v);h.showFile(v)}else h.file_input_hidden.val(q.fileName),h.file_link_wrapper.html(q.fileName);h.showMessage("success",_t("success"));4==this.readyState&&"function"==typeof h.complete_action&&h.complete_action()}else h.file_row_wrapper.remove(),h.showMessage("error",q.msg)}catch(w){h.file_row_wrapper.remove(),"undefined"==typeof q?h.showMessage("error",g.target.responseText):
h.showMessage("error",w)}};this.showMessage=function(g,m){h.parent_container.children("i").removeAttr("data-original-title");0<h.messages.length?("success"!=g?h.messages.push(m):h.messages.push("success"),g=0<h.messages.filter(q=>"success"==q).length?"fa fa-info-circle blue":"fa fa-exclamation-circle red",h.parent_container.find("[widget=tmultifile]").css("padding-left","20px"),h.parent_container.children("i").attr({"class":g+" file-response-icon",title:h.messages.filter(q=>"success"!=q).join(", "),
"data-original-title":h.messages.filter(q=>"success"!=q).join(", ")})):"success"==g?(h.parent_container.find("[widget=tmultifile]").css("padding-left","20px"),h.parent_container.children("i").attr({"class":"far fa-check-circle green file-response-icon",title:m,"data-original-title":m})):(h.messages.push(m),h.parent_container.find("[widget=tmultifile]").css("padding-left","20px"),h.parent_container.children("i").attr({"class":"fa fa-exclamation-circle red file-response-icon",title:m,"data-original-title":m}))};
this.clear=function(){$(".tfile_row_wrapper-"+a).each(function(){$(this).find("input").each(function(){$(this).val("")});$(this).remove()});h.parent_container.children("i").attr({"class":"",title:""});h.parent_container.find("[widget=tmultifile]").css("padding-left","5px");$('[name="file_'+this.field_name+'[]"]').val("");$(this.file_input_hidden).val("");$("#"+this.input_id).val("")};this.setValue=function(g){this.clear();if(g){try{g=JSON.parse(g)}catch(w){}for(var m in g){var q=g[m],v={idFile:m,
fileName:q};0<=q.indexOf("%7B%")&&(v=JSON.parse(decodeURIComponent(q)));this.file_handling?(this.createStatusRow(),this.setData(v),this.showFile()):this.createInputHidden(q)}}};this.updateValueInfo=function(){var g=new DataTransfer,m=h.instances.map(q=>q.obj_file);for(let q=0;q<m.length;q++)g.items.add(m[q]);$("#"+a)[0].files=g.files};this.addInstance=function(g){this.instances.push(g)}}
...
function tselect_clear_field(a,b){try{$("form[name="+a+'] [name="'+b+'[]"]').val("")}catch(c){console.log(c)}}function tselect_create_opt_group(a,b,c){a="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var e='[name="'+b+'[]"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(e="#"+b);$('<optgroup label="'+c+'"></optgroup>').appendTo(a+e)}
function tselect_add_option(a,b,c,e){c=c.replace(/"/g,"");a="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var k='select[name="'+b+'[]"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(k="#"+b);b=$(a+k).find("optgroup");0<b.length?$('<option value="'+c+'">'+e+"</option>").appendTo(b.last()):$('<option value="'+c+'">'+e+"</option>").appendTo(a+k)}
function tselect_clear(a,b,c){"undefined"==typeof c&&(c=!0);a="undefined"!=typeof a&&""!=a?'form[name="'+a+'"] ':"";var e='[name="'+b+'[]"]';-1==b.indexOf("[")&&0<$("#"+b).length&&(e="#"+b);b=$(a+e);b.val(!1);b.html("");c&&b.attr("changeaction")&&tform_events_hang_exec(b.attr("changeaction"))}
function tselect_enable_search(a,b){$(a).removeAttr("onchange");$(a).select2({allowClear:!0,multiple:!0,placeholder:b,templateResult:function(c){return/<[a-z][\s\S]*>/i.test(c.text)?$("<span>"+c.text+"</span>"):c.text},templateSelection:function(c){return/<[a-z][\s\S]*>/i.test(c.text)?$("<span>"+c.text+"</span>"):c.text}}).on("change",function(c){(new Function($(a).attr("changeaction")))()})};function tslider_start(a,b,c,e,k){b=$(a).val();$(a).wrap('<div class="tslidercontainer">');$(a).before('<div class="label">'+b+"</div>");$(a).on("input",function(){$(a).parent().find(".label").html(this.value)})}function tslider_enable_field(a,b){setTimeout(function(){$("form[name="+a+"] [name="+b+"]").css("pointer-events","auto");$("form[name="+a+"] [name="+b+"]").removeClass("tfield_disabled")},1)}
function tslider_disable_field(a,b){setTimeout(function(){$("form[name="+a+"] [name="+b+"]").css("pointer-events","none");$("form[name="+a+"] [name="+b+"]").addClass("tfield_disabled")},1)};function tsortlist_start(a,b,c,e){$(a).sortable({connectWith:b,tolerance:"pointer",start:function(k,d){d.item.data("index",d.item.index());d.item.data("parenta",this.id)},receive:function(k,d){0<e&&$(this).children().length>e?$(d.sender).sortable("cancel"):(k=$(this),targetListName=this.getAttribute("itemname"),document.getElementById(d.item.attr("id")+"_input").name=targetListName+"[]",d.item.css("display")!==k.attr("itemdisplay")&&(d.item.css("display",k.attr("itemdisplay")),detached=d.item.detach(),
...
$(a).parent().spinner("changing",function(k,d,f){$(a).attr("exitaction")&&(new Function($(a).attr("exitaction")))();"function"==typeof b&&b()})}function tspinner_enable_field(a,b){setTimeout(function(){$("form[name="+a+"] [name="+b+"]").parent().css("pointer-events","auto");$("form[name="+a+"] [name="+b+"]").removeClass("tfield_disabled");$("form[name="+a+"] [name="+b+"]").removeAttr("readonly")},1)}
...
minute:"2-digit"},eventTimeFormat:{hour12:!1,hour:"2-digit",minute:"2-digit"},editable:b,dayMaxEventRows:!0,events:d,eventDurationEditable:v,eventStartEditable:q,eventDisplay:"block",eventSourceSuccess:function(B){setTimeout(__adianti_process_popover,1500);return B.eventArray},eventContent:function(B){return{html:'<div class="fc-event-time">'+B.timeText+'</div><div class="fc-event-title-container"> <div class="fc-event-title fc-sticky">'+B.event.title+"</div> </div>"}},dateClick:function(B){if(""!==
f&&0==z&&0==A){var G=B.allDay?B.dateStr:moment(B.date).format("YYYY-MM-DD HH:mm:ss");__adianti_load_page(f+"&date="+G+"&view="+B.view.type)}},eventClick:function(B){""!==l&&0==z&&0==A&&__adianti_load_page(l+"&id="+B.event.id+"&key="+B.event.id+"&view="+B.view.type)},eventDragStart:function(){z=1},eventDragStop:function(){z=0},eventDrop:function(B){""!==p&&__adianti_ajax_exec(p+"&id="+B.event.id+"&key="+B.event.id+"&start_time="+moment(B.event.start).format("YYYY-MM-DD HH:mm:ss")+"&end_time="+moment(B.event.end).format("YYYY-MM-DD HH:mm:ss"))},
...
function ticonview_move_start(a,b,c,e){$("#"+a+" li"+c).draggable({zIndex:1E5,cursor:"pointer",start:function(k,d){$(this).css("opacity",.5);$(this).addClass("iconview-drag")},stop:function(k,d){$(this).css("opacity",1)},revert:function(k,d){$(this).data("uiDraggable").originalPosition={top:0,left:0};return!k}});$("ul.ticonview li"+e).droppable({accept:".iconview-drag",drop:function(k,d){k=$(d.draggable).data();d=$(this).data();var f={},l;for(l in k)!k.hasOwnProperty(l)||"string"!=typeof k[l]&&"number"!=
typeof k[l]||(f["source_"+l]=k[l]);for(l in d)!d.hasOwnProperty(l)||"string"!=typeof d[l]&&"number"!=typeof d[l]||(f["target_"+l]=d[l]);__adianti_load_page(b+"&"+$.param(f))},over:function(k,d){$(this).addClass("ui-over").addClass($(this).attr("id"))},out:function(k,d){$(this).removeClass("ui-over")}})};function kanban_start_board(a,b){$("#"+a).sortable({revert:200,placeholder:"kanban-stage-placeholder",forcePlaceholderSize:!0,helper:function(c,e){return $(e).clone().addClass("kanban-dragging")},start:function(c,e){e.item.show().addClass("kanban-ghost")},stop:function(c,e){e.item.show().removeClass("kanban-ghost")},update:function(c,e){c=$(e.item).attr("stage_id");var k=[];$(".kanban-stage").each(function(){k.push($(this).attr("stage_id"))});e=k.join("&order[]=");__adianti_load_page(b+"&key="+c+
...
var d=k.indexOf("");-1!==d&&k.splice(d,1);d=k.join("&order[]=");__adianti_load_page(b+"&key="+c+"&id="+c+"&stage_id="+e+"&order[]="+d)}},cursor:"move",tolerance:"pointer",cancel:".kanban-item-content, .kanban-item-actions"})}
...
function tbarcodeinputreader_open_reader(a){var b=a+"_reader",c=a+"_container",e=.9*$(window).width(),k=.8*$(window).height(),d=document.createElement("div"),f=document.createElement("div"),l=document.createElement("video");d.title="Barcode";d.id=c;l.id=b;f.id=b+"-cameras";d.setAttribute("class","tbarcodeinputreader_container");l.setAttribute("class","tbarcodeinputreader_reader");f.setAttribute("class","tbarcodeinputreader-cameras");d.appendChild(l);d.appendChild(f);$("#adianti_div_content").append(d);
tbarcodeinputreader_start(b,function(){tjquerydialog_start("#"+c,!0,!1,!1,e,k,0,0,99999,[],function(){tbarcodeinputreader.closing=!0;tbarcodeinputreader_stop()},!0,"")},function(p){__adianti_block_ui();$("#"+a).val(p);tbarcodeinputreader_stop();"undefined"!==typeof $("#"+a).attr("changeaction")&&Function($("#"+a).attr("changeaction"))();__adianti_unblock_ui()})}
function tbarcodeinputreader_start(a,b,c){tbarcodeinputreader=new ZXing.BrowserBarcodeReader;tbarcodeinputreader.closing=!1;tbarcodeinputreader._stopContinuousDecode=!1;tbarcodeinputreader_receive=c;tbarcodeinputreader.listVideoInputDevices().then(function(e){e&&0!=e.length&&(b(),1<e.length&&trbarcodeinputreader_make_radio_camera(a,e),tbarcodeinputreader_read(e[e.length-1].deviceId,a))}).catch(function(e){__adianti_error("Error",e)})}
...
function tqrcodeinputreader_open_reader(a){var b=a+"_reader",c=a+"_container",e=.9*$(window).width(),k=.8*$(window).height(),d=document.createElement("div"),f=document.createElement("div"),l=document.createElement("div");d.title="QRCode";d.id=c;l.id=b;f.id=b+"-cameras";d.setAttribute("class","tqrcodeinputreader_container");l.setAttribute("class","tqrcodeinputreader_reader");f.setAttribute("class","tqrcodeinputreader-cameras");d.appendChild(l);d.appendChild(f);$("#adianti_div_content").append(d);tqrcodeinputreader_start(b,
function(){tjquerydialog_start("#"+c,!0,!1,!1,e,k,0,0,99999,[],tqrcodeinputreader_stop,!0,"")},function(p){__adianti_block_ui();$("#"+a).val(p);tqrcodeinputreader_stop();"undefined"!==typeof $("#"+a).attr("changeaction")&&Function($("#"+a).attr("changeaction"))();__adianti_unblock_ui()})}
function tqrcodeinputreader_start(a,b,c){tqrcodeinputreader=new Html5Qrcode(a);tqrcodeinputreader_receive=c;Html5Qrcode.getCameras().then(function(e){e&&0!=e.length&&(b(),1<e.length&&trqcodeinputreader_make_radio_camera(a,e),tqrcodeinputreader_read(e[e.length-1].id))}).catch(function(e){__adianti_error("Error",e)})}
...
"image"+Math.floor(1E6*Math.random()+1)+".png",{type:g.type?g.type:"image/png"})},u=function(){__adianti_block_ui();var t=g.cropper.getCroppedCanvas({width:g.config.cropWidth,heigth:g.config.cropHeight}).toDataURL(g.type);$("#timagecropper_"+g.field).attr("src",t);$("#timagecropper_"+g.field).show();g.actions.show();g.placeholder.hide();if(g.base64)g.file_input_hidden.val(t),__adianti_unblock_ui();else try{var r=w(t),x=new FormData;x.append("fileName",r);var y=new XMLHttpRequest;y.open("POST",g.serviceAction,
...
return}if("200"==r&&"4"==t.target.readyState&&t.target.responseText)try{__adianti_unblock_ui();var x=JSON.parse(t.target.responseText);"error"==x.type&&__adianti_error("Error",x.msg);if(g.fileHandling){var y=E();y.fileName&&y.fileName!=y.newFile&&(y.delFile=y.fileName);y.newFile="tmp/"+x.fileName;y.fileName="tmp/"+x.fileName;A(y)}else g.file_input_hidden.val(g.name)}catch(C){__adianti_unblock_ui(),__adianti_error("Error",C)}},A=function(t){t?$(g.file_input_hidden).val(encodeURIComponent(JSON.stringify(t))):
...
t.setAttribute("class","fa fa-arrows-alt-v");r.setAttribute("title",g.config.labels.scaley);r.setAttribute("data-option","-1");r.addEventListener("click",function(){K(this)});r.addEventListener("touchstart",function(){K(this)});r.appendChild(t);return r},J=function(t){var r=t.getAttribute("data-option");g.cropper.scaleX(r);t.setAttribute("data-option",-1*r)},K=function(t){var r=t.getAttribute("data-option");g.cropper.scaleY(r);t.setAttribute("data-option",-1*r)},L=function(){var t=document.createElement("i"),
...
function tgantt_set_update_action(a,b){var c=$(a).data("pixelStep");$(a+" .tgantt-event").draggable({addClasses:!1,delay:50,axis:"x",grid:[c,100],drag:function(e,k){tgantt_update_data_title(a,k)},stop:function(e,k){e.stopPropagation();if(0>k.position.left){var d=k.helper.width();e=Math.abs(k.position.left);var f=parseInt(k.helper.css("margin-left"));d+(f-e)<=c&&(e=-1*(f+d)+parseFloat(c),k.helper.css("left",e+"px"),k.position.left=e)}else d=k.helper.parent().parent().width(),e=Math.abs(k.position.left),
k.helper.width(),f=parseInt(k.helper.css("margin-left")),e+f>=d&&(e=d-f-parseFloat(c),k.helper.css("left",e+"px"),k.position.left=e);d=tgantt_get_time_drag(a,k);e=JSON.parse(base64_decode(k.helper.attr("data")));var l=new Date(e.start_time);l.setTime(l.getTime()+36E5*d);f=new Date(e.end_time);f.setTime(f.getTime()+36E5*d);d=e.id;l=tgantt_format_date(l);f=tgantt_format_date(f);e.start_time=l;e.end_time=f;k.helper.attr("data",base64_encode(JSON.stringify(e)));__adianti_ajax_exec(b+"&id="+d+"&key="+
d+"&start_time="+l+"&end_time="+f)}}).resizable({addClasses:!1,delay:100,handles:"e,w",axis:"x",grid:[c,10],stop:function(e,k){e.stopPropagation();e.preventDefault();var d=$(this).data("ui-resizable").axis,f="e"==d?0<$(this).data("ui-resizable").size.width-$(this).data("ui-resizable").originalSize.width?"right":"left":0>$(this).data("ui-resizable").size.width-$(this).data("ui-resizable").originalSize.width?"right":"left",l=tgantt_get_time_resize(a,k);e=JSON.parse(base64_decode(k.helper.attr("data")));
var p=e.id;"w"==d?(d=new Date(e.start_time),"left"==f?d.setTime(d.getTime()-36E5*Math.abs(l)):d.setTime(d.getTime()+36E5*Math.abs(l)),f=tgantt_format_date(d),l=e.end_time):(d=new Date(e.end_time),"left"==f?d.setTime(d.getTime()-36E5*Math.abs(l)):d.setTime(d.getTime()+36E5*Math.abs(l)),f=e.start_time,l=tgantt_format_date(d));e.start_time=f;e.end_time=l;k.helper.attr("data",base64_encode(JSON.stringify(e)));__adianti_ajax_exec(b+"&id="+p+"&key="+p+"&start_time="+f+"&end_time="+l)}})}
function tgantt_update_data_title(a,b){if(1>$(".tooltip").length)return!1;b=tgantt_get_time_drag(b);var c=$(".tooltip"),e=c.position().left,k=$(a).data("lastHour"),d=$(a).data("pixelStep");if(k==b)return!1;b>k?c.css("left",e+parseInt(d)+"px"):c.css("left",e-d+"px");$(a).data("lastHour",b)}function tgantt_set_height(a){let b=[];$(a+" .table-content tr").each(function(){b.push($(this).outerHeight())});b.reverse();$(a+" .table-rows tr").each(function(){$(this).outerHeight(b.pop())})}
function tgantt_get_steps(a){return a.position.left-a.originalPosition.left}function tgantt_get_time_drag(a,b){b=tgantt_get_steps(b);var c=$(a).data("pixelStep");a=$(a).data("minuteStep");return 0==b?0:b/c*a/60}function tgantt_get_time_resize(a,b){var c=tgantt_get_time_drag(a,b),e=$(a).data("pixelStep");a=$(a).data("minuteStep");if(0!=c)return c;b=b.size.width-b.originalSize.width;return 0==b?0:b/e*a/60};Adianti.translation={en:{large_file:"The file is too large. The upload limit is ",success:"Success",file_uploaded_successfully:"File uploaded successfully"},pt:{large_file:"O arquivo \u00e9 muito grande. O limite de carregamento \u00e9 ",success:"Sucesso",file_uploaded_successfully:"Arquivo carregado com sucesso"},es:{large_file:"El archivo es demasiado grande. El l\u00edmite de carga es ",success:"\u00c9xito",file_uploaded_successfully:"Archivo cargado con \u00e9xito"}};
...
d.data("select2").$dropdown;if(u&&0!==u.length){var w=u.find(".select2-results__option:visible .select2-result-item__checkbox[data-option-value]");0!==w.length&&(w.each(function(z){if(z=$(this).attr("data-option-value"))if(z=d.find(`option[value="${z}"]`),0!==z.length){var A=!z.prop("selected");z.prop("selected",A);$(this).prop("checked",A);$(this).closest(".select2-results__option").attr("aria-selected",A?"true":"false")}}),d.trigger("change"),bselectcheck_update_summary(d,b),setTimeout(function(){c(d)},
100))}}catch(u){console.error("ERRO DURANTE INVERS\u00c3O:",u)}finally{e=!1}});setTimeout(function(){c(d)},100);h.on("input",function(){var v=$(this).val().toLowerCase();l.find(".select2-results__options li").each(function(){$(this).hasClass("select2-results__message")||($(this).text().toLowerCase().includes(v)?$(this).show():$(this).hide())})});setTimeout(function(){h.focus()},0)});d.on("select2:select select2:unselect",function(l){var p=l.params.data.selected;"undefined"!=typeof l.params.originalEvent&&
...
function bselectcheck_update_summary(a,b){var c=$(a);a=c.select2("data");const e=a?.length,k=c.next(".select2-container");c=k.find(".select2-selection__rendered");if(0===e)c.find("li:not(.select2-search--inline)").remove(),c.find(".select2-selection__choice, .select2-selection__clear").remove(),c.prepend(`<li class="select2-selection__placeholder">${b}</li>`);else{const {visibleText:d,remainingCount:f}=bselectcheck_calculate_visible_items(k,a);b=d;0<f&&(b+=` (+${f} itens)`);c.find("li:not(.select2-search--inline)").remove();
c.prepend(`<span style="width: 100%;max-width: 100%;"class="select2-selection__choice">${b}</span>`)}}
function bselectcheck_calculate_visible_items(a,b){const c=$("<span>").css({"font-size":a.css("font-size"),"font-family":a.css("font-family"),visibility:"hidden",position:"absolute","white-space":"nowrap"}).appendTo("body");a=a.width()-80;let e=[],k=0;b=b?.filter(d=>d&&d.text?.trim());for(let d=0;d<b?.length;d++){const f=0===e.length?b[d].text.trim():e.join(", ")+", "+b[d].text.trim();c.text(d<b.length-1?f+` (+${b.length-d-1} itens)`:f);if(a<c.width()){k=b.length-d;break}e.push(b[d].text.trim())}c.remove();
return{visibleText:0<e.length?e.join(", "):"",remainingCount:k}};
